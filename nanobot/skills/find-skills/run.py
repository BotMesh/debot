...
def list_skills(workspace: Path | None = None) -> dict:
    ...
    # system bundled
    system = sorted([p.name for p in pkg_skills.iterdir() if p.is_dir() and (p / "SKILL.md").exists()])
    skills["system"] = system

    # workspace
    ws_dir = get_skills_path(workspace)
    if ws_dir.exists():
        workspace_skills = sorted([p.name for p in ws_dir.iterdir() if p.is_dir() and (p / "SKILL.md").exists()])
    else:
        workspace_skills = []
